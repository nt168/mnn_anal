...
def extract_unimodel_data(result_type):
    """
    从数据库提取task1 P/N组合性能数据

    Args:
        result_type: 'pp' 或 'tg'
    Returns:
        DataFrame: 包含性能数据的DataFrame
    """
...
def main():
    """Main函数"""
    print("🚀 P/N组合基准测试3D散点图动画生成器")
    print("=" * 60)

    script_dir = Path(__file__).parent
    output_dir = script_dir / ".." / "analysis_output" / "unimodel_3d_animation"
    ...
    for task_type, task_name in tasks:
        ...
        # 提取数据
        df = extract_unimodel_data(task_type)
        if df is not None:
