...
//    static const std::string build_commit;
    ...
    std::string              pType;              // 新增：提示词类型 (fix/variable/file)
    ...
    TestInstance(const commandParametersInstance & instance) {
        ...
        // 新增：判断pType类型，pf优先级高于vp
        if (!instance.mCmdParam.promptFilePath.empty()) {
            pType = "file";
...
// 新增：显示token向量详细信息
static void displayTokensInfo(const std::vector<int>& promptTokens, int decodeCount, bool isVariablePrompt, int testToken) {
    printf("=== Prompt Test Information ===\n");
    printf("Test Mode: %s\n", isVariablePrompt ? "Variable Prompt" : "Fixed Prompt");
    printf("Test Token: %d\n", testToken);
    printf("File Input: %s\n", promptTokens.empty() ? "No" : "Yes");
    printf("File Length: %d\n", promptTokens.empty() ? 0 : (int)promptTokens.size());
    printf("Original nPrompt: %d\n", isVariablePrompt ? testToken : 16);
    printf("Prompt Tokens Vector [size=%zd]: [", promptTokens.size());
    for (size_t i = 0; i < promptTokens.size(); ++i) {
        if (i > 0) printf(", ");
        printf("%d", promptTokens[i]);
        if (promptTokens.size() > 10 && i >= 4 && i < promptTokens.size() - 5) {
            printf(" ... %d more", (int)(promptTokens.size() - 10));
            break;
        }
    }
    printf("]\n");

    printf("Decode Token Count: %d\n", decodeCount);
    printf("Note: Decode tokens are generated dynamically during inference\n");
    printf("================================\n");
}

// 新增：显示decode token解码后的实际文字内容
static void displayDecodeTokens(const std::vector<int>& decodeTokens, Llm* llm, int maxDisplay = 100) {
    if (decodeTokens.empty() || !llm) {
        return;
    }

    printf("\n=== Generated Content (Decode Tokens Decoded) ===\n");
    printf("Decode Tokens [size=%zd]: [", decodeTokens.size());

    // 显示tokenID序列（前10个）
    for (size_t i = 0; i < std::min((size_t)10, decodeTokens.size()); ++i) {
        if (i > 0) printf(", ");
        printf("%d", decodeTokens[i]);
    }
    if (decodeTokens.size() > 10) {
        printf(" ... %ld more", (long)(decodeTokens.size() - 10));
    }
    printf("]\n");
...
// 新增：从tokenizer文件获取vocab大小
static int getVocabSize(const std::string& config_path) {
    // 获取config文件所在目录
    ...
    std::string dir = config_path.substr(0, pos);

    // 尝试打开tokenizer.txt文件
    std::string tokenizer_path = MNNFilePathConcat(dir, "tokenizer.txt");
    std::ifstream tokenizer_file(tokenizer_path.c_str());

    if (!tokenizer_file.is_open()) {
        // MNN_PRINT("Warning: Cannot open tokenizer.txt at %s, using default vocab size 1000\n", tokenizer_path.c_str());
        return 1000; // 默认值
    }
    ...
    // MNN_PRINT("Loaded vocab size: %d from %s\n", line_count, tokenizer_path.c_str());
    return line_count - 1; // 减1，因为token从0开始
}
...
int main(int argc, char ** argv) {
    ...
    for (const auto & instance: paramsInstances) {
        ...
        // 新增：使用正式tokenizer加载文件tokens作为原料
        if (!t.promptFilePath.empty()) {
            fileTokens = loadTokensFromFile(t.promptFilePath, t.modelConfigFile, llm.get());
            if (fileTokens.empty()) {
        ...
        if (instance.mCmdParam.kvCache == "true") {
            // 使用文件tokens或生成的tokens
            std::vector<int> tokens;
            if (!t.promptFilePath.empty()) {
                // 根据参数长度调整文件tokens
                tokens = adjustTokensToLength(fileTokens, instance.mCmdParam.nPrompt);
                if (t.verbose) {
                    printf("DEBUG: Using file tokens for llm_demo test\n");
                }
            } else {
                // 使用生成的tokens，基于实际的prompt长度
                if (t.useVariablePrompt) {
                    for (int i = 0; i < t.nPrompt; ++i) {
                        tokens.push_back(20 + (i % 20)); // 20-39范围的循环
                    }
                } else {
                    tokens = std::vector<int>(t.nPrompt, 16);
                }
            }
            int testToken = tokens.empty() ? 16 : tokens[0];  // 用于显示的测试token
            std::vector<int> decodeVectors(1, testToken);

            // 新增：如果开启verbose，显示token信息
            if (t.verbose) {
                displayTokensInfo(tokens, decodeTokens, t.useVariablePrompt, testToken);
            }
            ...
            for (int i = 0; i < instance.mCmdParam.nRepeat + 1; ++i) {
                ...
                // 新增：如果开启verbose，显示实际生成的decode内容
                if (t.verbose && i == instance.mCmdParam.nRepeat + 1) { // 只在最后一次显示
                    auto outputTokens = context->output_tokens;
                    if (!outputTokens.empty() && outputTokens.size() >= instance.mCmdParam.nGenerate) {
        ...
        if (instance.mCmdParam.kvCache == "false") {
            // 使用文件tokens或生成的tokens
            std::vector<int> tokens;
            if (!t.promptFilePath.empty() && instance.mCmdParam.nPrompt > 0) {
                // 根据参数长度调整文件tokens用于prompt测试
                tokens = adjustTokensToLength(fileTokens, instance.mCmdParam.nPrompt);
            } else if (instance.mCmdParam.nPrompt > 0) {
                // 使用生成的tokens，基于原始prompt长度
                if (t.useVariablePrompt) {
                    for (int i = 0; i < instance.mCmdParam.nPrompt; ++i) {
                        tokens.push_back(20 + (i % 20)); // 20-39范围的循环
                    }
                } else {
                    tokens = std::vector<int>(instance.mCmdParam.nPrompt, 16);
                }
            }
            int testToken = tokens.empty() ? 16 : tokens[0];  // 用于显示的测试token
            std::vector<int> decodeVectors(1, testToken);
            ...
            if (t.verbose) {
                if (t.nPrompt > 0) {
                    ...
                    displayTokensInfo(std::vector<int>(), decodeTokens, t.useVariablePrompt, testToken);
                }
            }

            for (int i = 0; i < instance.mCmdParam.nRepeat + 1; ++i) {
                int64_t sampler_us = 0;
                if (instance.mCmdParam.nPrompt > 0) {
                    // 根据参数长度调整文件tokens或使用生成的tokens
                    std::vector<int> actualTokens;
                    int actualPromptLength = instance.mCmdParam.nPrompt; // 始终使用参数指定的长度
                    if (!t.promptFilePath.empty()) {
                        actualTokens = adjustTokensToLength(fileTokens, actualPromptLength);
                    } else {
                        actualTokens = tokens;
                    }
                    if (actualTokens.empty()) {
                        llm->response(std::vector<int>(actualPromptLength, 16), nullptr, nullptr, 1);
                    } else {
                        llm->response(actualTokens, nullptr, nullptr, 1);
                    }
                    sampler_us += context->prefill_us;
                }
                if (instance.mCmdParam.nGenerate > 0) {
                    llm->response(decodeVectors, nullptr, nullptr, instance.mCmdParam.nGenerate);
                ...
                // 新增：如果开启verbose且用于generate测试，收集实际的输出tokens
                std::vector<int> actualOutputTokens;
                if (t.verbose && instance.mCmdParam.nGenerate > 0) {
