...
// 显示测试配置信息（命令行参数等）
static void displayTestConfiguration(const TestInstance& t, const std::string& kvCache, const std::string& promptFilePath) {
    printf("\n=== Test Configuration ===\n");
    printf("Model: %s\n", t.model.c_str());
    printf("Backend: %d\n", t.backend);
    printf("Threads: %d\n", t.threads);
...
int main(int argc, char ** argv) {
    ...
    if (!paramsInstances.empty() && paramsInstances[0].mCmdParam.verbose) {
        ...
        displayTestConfiguration(firstInstance, paramsInstances[0].mCmdParam.kvCache, paramsInstances[0].mCmdParam.promptFilePath);
    }
    ...
    for (const auto & instance: paramsInstances) {
        ...
        // 在有prompt文件的情况下，使用正式tokenizer加载文件tokens作为原料
